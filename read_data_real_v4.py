...
file_path = os.path.join(dir_path, "data", "Enoch-TCCUSP-V4-Enviado.xlsx")
xls = pd.ExcelFile(file_path)

localidade = pd.read_excel(xls, 'Localidade')
capacidades= pd.read_excel(xls, 'Capacidades')
coleta = pd.read_excel(xls, 'Coleta')
malha = pd.read_excel(xls, 'Malha')
realizado = pd.read_excel(xls, 'Realizado')

####################
# Locations
####################

localidade = localidade.append(localidade[localidade['Tipo Localidade']=='BASE'].iloc[0], ignore_index=True) # duplicate CD
...
horizon = coleta['DATA ENTREGA'].max()+1
work_shift = 10
rest_shift = 14
day = 24
...
nodes = pd.merge(coleta, localidade, how='left', left_on=['DESTINO'], right_on=['Localidade'])
nodes = nodes.reset_index()
nodes['index'] = nodes['index'] + 1 # 1-index instead of 0-index, as we will add the deposit
nodes['Localidade'] = nodes['DESTINO']

# nodes['start_window'] = day*(nodes['Dia']-1) + (nodes['inicio'] - start_hour) 
# nodes['end_window'] = day*(nodes['Dia']-1) + (nodes['fim'] - start_hour)
nodes['start_window'] = nodes['Horario Inicio']
nodes['end_window'] = nodes['Horario Fim']
nodes.drop(columns=['ORIGEM', 'DESTINO', 'Tipo Localidade', 'Horario Inicio', 'Horario Fim', 'Tempo carga', 'Tempo Descarga'], inplace=True)
cd_node_0 = {
    'index'         :         [0],
    'M3'            :         [0],
    'Valor'         :         [0],
    'Localidade'    :         ['CD'],
    'service_time'  :         [localidade.loc[localidade['Localidade']=='CD', 'service_time'].iloc[0]],
    'start_window'  :         [0],
    'end_window'    :         [localidade.loc[localidade['Localidade']=='CD', 'Horario Fim'].iloc[0]*horizon],
}
cd_node_last = {
    'index'         :         [n+1],
    'M3'            :         [0],
    ...
    'service_time'  :         [0],
    'start_window'  :        [0],
    'end_window'    :        [localidade.loc[localidade['Localidade']=='CD', 'Horario Fim'].iloc[0]*(2*horizon)],
} # 2 times the horizon is an arbitrary value, just to say that there's no restriction. A tighter bound can be found
...
speed = 60
arcs['Tempo Corrido'] = arcs['Distancia (km)'] / speed
arcs.drop(columns=['Tempo (h)'], inplace=True)
arcs.rename(columns={'Tempo Corrido':'Tempo (h)'}, inplace=True)
# make the arcs take into account the multiplicity of each client
# missing clients to deposit arcs, so add them artificially

to_deposit = arcs[arcs['Origem']=='CD'].copy(deep=True)
to_deposit.rename(columns={'Origem':'Destino', 'Destino':'Origem'}, inplace=True)

arcs=pd.concat([arcs, to_deposit], axis=0, sort=True)
...
if len(mod_indexes)>0:
    ...
    cyc = cycle(range(1, horizon))
    for i, v in enumerate(mod_indexes):
        nodes.loc[nodes['index']==v, 'DATA ENTREGA'] = next(cyc)
...
start_windows = nodes['start_window'].values
end_windows = nodes['end_window'].values
...
reference = realizado.copy(deep=True)
reference = pd.merge(reference, nodes[['Localidade','index']], how='left', left_on='Loja', right_on='Localidade')
